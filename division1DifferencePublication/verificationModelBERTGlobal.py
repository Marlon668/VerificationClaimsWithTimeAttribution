...
'''
Global version of extension 1: adding buckets of publicationtime
...
class verifactionModel(nn.Module):
    ...
    def forward(self, claim, evidences, metadata_encoding, domain, claimDate, snippetDates):
        ...
        claim_encoding = self.alpha * sentence_embeddings + (1 - self.alpha) * self.claimDate(
        ...
        for i in range(len(evidences)):
            ...
            # Perform pooling
            sentence_embeddings = self.mean_pooling(model_output, encoded_input['attention_mask']).to(self.device)
            evidence_encoding = self.alpha  * sentence_embeddings + (1 - self.alpha) * self.evidenceDate(
    ...
    def forwardAttribution(self, claim, evidences, metadata_encoding, domain, claimDate, snippetDates):
        ...
        claim_encodingFull = self.alpha * claim_EncodingWithoutTime + (1 - self.alpha) * self.claimDate(
            torch.tensor([claimDate]).to(self.device)).squeeze(0)
        ...
        for i in range(len(evidences)):
            ...
            evidence_EncodingWithoutTime = self.mean_pooling(model_output, encoded_input['attention_mask']).to(
                self.device)
            evidence_encoding = self.alpha * evidence_EncodingWithoutTime + (1 - self.alpha) * self.evidenceDate(
                torch.tensor([int(snippetDates[i + 1])]).to(self.device)).squeeze(0)
    ...
    def forwardIntegrated(self, inputs, metadata_encoding):
        claimEncoding = self.alpha * inputs[0] + (1 - self.alpha) * inputs[1]
        distribution = torch.zeros(len(self.labelDomains[self.domain])).to(self.device)
        for i in range(len(inputs[2])):
            evidence_encoding = self.alpha * inputs[2][i][0] + (1 - self.alpha) * inputs[2][i][1]
...
if __name__ == "__main__":
    ...
    for domain in domains:
        ...
        verificationModelM = verifactionModel(transformer,encoderMetadataM, instanceEncoderM,
                                            evidenceRankerM,
                                            labelEmbeddingLayerM,labelMaskDomainM, domainIndices,domainWeights,domain,sys.argv[2]).to(device)

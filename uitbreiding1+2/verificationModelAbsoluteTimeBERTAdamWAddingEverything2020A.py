...
class verifactionModel(nn.Module):
        ...
        if verschillenIndices[0] != "":
            for i in range(len(verschillenIndices)):
                ...
                if i + 1 >= len(verschillenValues):
                    break
        if number > 0:
            claim_encoding = alpha*claim_encoding + beta* self.claimDate(torch.tensor([claimDate]).to(self.device)).squeeze(0).to(self.device) + (1-alpha-beta)*tijdAbsolute/number
            ...
            if timeRefsSnippet[0] != "":
                for j in range(len(timeRefsSnippet)):
                    ...
                    if j + 1 >= len(timeHeidelSnippet):
                        break
            if number > 0:
                evidence_encoding = alpha*evidence_encoding+ beta*self.evidenceDate(torch.tensor([int(snippetDates[i+1])]).to(self.device)).squeeze(0).to(self.device) + (1-alpha-beta)*tijdAbsolute/number
        ...
        if verschillenIndices[0] != "":
            for i in range(len(verschillenIndices)):
                ...
                if i + 1 >= len(verschillenValues):
                    break
        if number > 0:
            claim_encoding = alpha * claim_encoding + beta * self.claimDate(
                torch.tensor([claimDate]).to(self.device)).squeeze(0).to(self.device) + (
                                         1 - alpha - beta) * tijdAbsolute / number
            ...
            if timeRefsSnippet[0] != "":
                for j in range(len(timeRefsSnippet)):
                    ...
                    if j + 1 >= len(timeHeidelSnippet):
                        break
            if number > 0:
                evidence_encoding = alpha * evidence_encoding + beta * self.evidenceDate(
    ...
    def saving_NeuralNetwork(model):
        pathI = "D:/models/iteratie2/modelsBERTAbsoluteAdamWEverything2020A/" + model.domain
...
def preprocessing(models,epochs=800):
    with torch.no_grad():
        for model in models:
            ...
            file = open('/scratch/leuven/347/vsc34763/finalResults/iteratie2/lossAbsoluteBertAdamWEverything2020A/' + 'Loss' + model[3], 'a', encoding='utf-8')
            ...
            if not model[8].early_stop:
                if model[8].better:
                    file = open('/scratch/leuven/347/vsc34763/finalResults/iteratie2/epochsAbsoluteBertAdamWEverything2020A/' + model[3], 'w', encoding='utf-8')

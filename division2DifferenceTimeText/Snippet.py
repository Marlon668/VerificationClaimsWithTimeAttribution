...
class snippet:
    ...
    def readOpenInformationExtraction(self,path=None):
        if path is None:
            path = os.pardir+"/OpenInformation" + "/" + self.claimID + "/" + self.number
        else:
            path += "/" + self.claimID + "/" + self.number
    ...
    '''
    def readTime(self,path=None):
        if path is None:
            path = os.pardir+"/processedTimes" + "/" + self.claimID + "/" + self.number+".xml"
        else:
            path = path+"/" + self.claimID + "/" + self.number+".xml"
        ...
        try:
            ...
            if snippet.optional != None:
                if not (os.path.exists(path)):
                    os.mkdir(path)
                f = open(path + "/" + snippet.number, "w", encoding="utf-8")
    ...
    '''
    def readPublicationDate(self,path=None):
        if path is None:
            path = os.pardir + '/SnippetDates' + '/' + self.claimID + '/' + self.number + '.xml'
        else:
            path += '/'+ self.claimID + '/' + self.number + '.xml'
        try:

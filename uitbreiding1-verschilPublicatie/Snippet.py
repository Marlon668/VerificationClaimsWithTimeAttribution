...
class snippet:
    ...
    def readTime(self):
        path = "ProcessedTimes2" + "/" + self.claimID + "/" + self.number+".xml"
        if os.path.exists(path):
            ...
            for index in range(len(newSplits)):
                if newSplits[index].find('tid=') != -1:
                    #print("Split")
                    #print(newSplits[index].split('>')[1])
                    datas.append(newSplits[index].split('>')[1])
                    indices.append(index)
                else:
                    #print("Split not")
                    #print(newSplits[index])
    ...
    def processDate(self):
        try:
            ...
            path = 'processedSnippets' + '/' + self.claimID+'/'+self.number + '.xml'
            if os.path.exists(path):
                parser = etree.XMLParser(recover=True)
                tree = etree.parse(path, parser)
                root = tree.getroot()
                                                            ...
                                                            self.publishTime = tuple(
                                                                [datetime.datetime.strptime(dateB,
                                                                 ...
                                                                 datetime.datetime.strptime(dateE,
                                                                                            '%Y-%m-%d')])
                                                            # print('Claim ' + self.claimID)
                                                            #self.publishTime = datetime.datetime.strptime(root[0].attrib['value'], '%Y-%m')
                                                        else:
                                                            if len(root[0].attrib['value']) == 4:
                                                                ...
                                                                self.publishTime = tuple(
                                                                    [datetime.datetime.strptime(dateB,
                                                                     ...
                                                                     datetime.datetime.strptime(dateE,
                                                                                                '%Y-%m-%d')])
                                                                #self.publishTime = datetime.datetime.strptime(root[0].attrib['value'], '%Y')
                                                            else:

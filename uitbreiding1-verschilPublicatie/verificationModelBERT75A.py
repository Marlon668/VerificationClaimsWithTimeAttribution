...
from transformers import BertTokenizer, AutoModel, AutoTokenizer, get_linear_schedule_with_warmup, AdamW
...
class verifactionModel(nn.Module):
    ...
    def forward(self, claim, evidences, metadata_encoding, domain, claimDate, snippetDates):
        ...
        # Perform pooling
        sentence_embeddings = self.mean_pooling(model_output, encoded_input['attention_mask']).to(self.device)
        alpha = 0.75

        claim_encoding = alpha * sentence_embeddings + (1 - alpha) * self.claimDate(
            torch.tensor([claimDate]).to(self.device)).squeeze(0)
        ...
        for i in range(len(evidences)):
            ...
            # Perform pooling
            sentence_embeddings = self.mean_pooling(model_output, encoded_input['attention_mask']).to(self.device)
            evidence_encoding = alpha * sentence_embeddings + (1 - alpha) * self.evidenceDate(
                torch.tensor([int(snippetDates[i + 1])]).to(self.device)).squeeze(0)
    ...
    def saving_NeuralNetwork(model):
        pathI = "D:/models/iteratie1/modelsBERT75A/" + model.domain
    ...
    def forwardAttribution(self, claim, evidences, metadata_encoding, domain, claimDate, snippetDates):
        ...
        model_output = self.transformer(**encoded_input)
        # Perform pooling
        claim_EncodingWithoutTime = self.mean_pooling(model_output, encoded_input['attention_mask']).to(self.device)
        alpha = 0.75
        claim_encodingFull = alpha * claim_EncodingWithoutTime + (1 - alpha) * self.claimDate(
            torch.tensor([claimDate]).to(self.device)).squeeze(0)
        timeClaim = self.claimDate(torch.tensor([claimDate]).to(self.device)).squeeze(0)
        ...
        for i in range(len(evidences)):
            ...
            model_output = self.transformer(**encoded_input)
            # Perform pooling
            evidence_EncodingWithoutTime = self.mean_pooling(model_output, encoded_input['attention_mask']).to(
                self.device)
            evidence_encoding = alpha * evidence_EncodingWithoutTime + (1 - alpha) * self.evidenceDate(
                torch.tensor([int(snippetDates[i + 1])]).to(self.device)).squeeze(0)
            timeEvidence = self.evidenceDate(
                torch.tensor([int(snippetDates[i + 1])]).to(self.device)).squeeze(0)
            evidenceEncodings.append((evidence_encoding, evidence_EncodingWithoutTime, timeEvidence))
            instance_encoding = self.instanceEncoder(claim_encodingFull.squeeze(0), evidence_encoding.squeeze(0),
                                                     metadata_encoding.squeeze(0)).to(self.device)
            rank_evidence = self.evidenceRanker(instance_encoding).to(self.device)
            label_distribution = self.labelEmbedding(instance_encoding, domain).to(self.device)
            label_distributionDomain = self.labelMaskDomain(label_distribution).to(self.device)
            distribution = distribution + torch.mul(rank_evidence, label_distributionDomain).to(self.device)
        return distribution.to(self.device), claim_encodingFull, claim_EncodingWithoutTime, timeClaim, evidenceEncodings

    def forwardIntegrated(self, inputs, metadata_encoding):
        alpha = 0.75
        claimEncoding = alpha * inputs[0] + (1 - alpha) * inputs[1]
        # print("Claim")
        # print(claimEncoding)
        distribution = torch.zeros(len(self.labelDomains[self.domain])).to(self.device)
        for i in range(len(inputs[2])):
            evidence_encoding = alpha * inputs[2][i][0] + (1 - alpha) * inputs[2][i][1]
            # print("Evidence")
            # print(evidence_encoding)
            instance_encoding = self.instanceEncoder(claimEncoding, evidence_encoding,
                                                     torch.tensor([0, 0, 0]).squeeze(0)).to(self.device)
            # print("Instantie")
    ...
    def getBaseLine(self, encodingClaim, encodingEvidence, metadata):
        with torch.no_grad():
            ...
            low, high = -0.25, 0.25
            while (abs(differenceToZero) > 1e-7):
    ...
    def getClaimEncoding(self, claim):
        ...
        # Perform pooling
        claim_EncodingWithoutTime = self.mean_pooling(model_output, encoded_input['attention_mask']).to(self.device)
        return claim_EncodingWithoutTime
    ...
    def getSnippetEncodings(self, evidences, snippetDates):
        ...
        for i in range(len(evidences)):
            ...
            evidence_EncodingWithoutTime = self.mean_pooling(model_output, encoded_input['attention_mask']).to(
                self.device)
            evidenceEncodings = torch.cat((evidenceEncodings, evidence_EncodingWithoutTime))
...
def preprocessing(models,epochs=800):
    with torch.no_grad():
        for model in models:
            ...
            file = open('/scratch/leuven/347/vsc34763/finalResults/iteratie1/lossBert75A/' + 'Loss' + model[3], 'a',
                        encoding='utf-8')
            file.write("Preprocessing stap " + str(0) + " - " + str(validation_loss) + "\n")
            file.close()
            if not model[8].early_stop:
                if model[8].better:
                    file = open('/scratch/leuven/347/vsc34763/finalResults/iteratie1/epochsBert75A/' + model[3], 'w', encoding='utf-8')
...
if __name__ == "__main__":
    ...
    #domains = {'ranz','huca','mpws','peck','clck','fani','obry','faly','thet','thal','bove','para','wast'}

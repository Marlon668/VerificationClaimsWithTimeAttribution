...
from uitbreiding2VerschilTijdTekst.pytorchtools import EarlyStopping

import torch.nn.functional as F
import torch
from torch.utils.data import DataLoader

from uitbreiding2VerschilTijdTekst.labelMaskDomain import labelMaskDomain
'''
combining pre-created partitions into one dataset
'''
from uitbreiding2VerschilTijdTekst.datasetIteratie2Combiner import dump_load, dump_write, NUS
'''
...
'''
from uitbreiding2VerschilTijdTekst.encoderMetadata import encoderMetadata
from uitbreiding2VerschilTijdTekst.evidence_ranker import evidenceRanker
from uitbreiding2VerschilTijdTekst.instanceEncoder import instanceEncoder
from uitbreiding2VerschilTijdTekst.labelEmbeddingLayer import labelEmbeddingLayer
from uitbreiding2VerschilTijdTekst.OneHotEncoder import oneHotEncoder
from torch import nn
from sklearn.metrics import f1_score

from uitbreiding2VerschilTijdTekst.encoderGlobal import encoder
...
class verifactionModel(nn.Module):
    ...
    def __init__(self,encoder,metadataEncoder,instanceEncoder,evidenceRanker,labelEmbedding,labelMaskDomain,labelDomains,domainWeights,domain,alpha=0.25):
    ...
    def forwardAttribution(self, claim, evidences, metadata_encoding, domain, claimDate, snippetDates, verbsClaim,
                           timeExpressionsClaim, positionClaim, timeRefsClaim,
                           timeHeidelClaim, verbsSnippets, timeExpressionsSnippets, positionSnippets, timeRefsSnippets,
                           timeHeidelSnippets):
        claim_encodingFull, claim_EncodingWithoutTime, times = \
            self.encoder.forwardAttribution(claim, claimDate,positionClaim.split('\t'),
                                            verbsClaim.split('\t'),timeExpressionsClaim.split('\t'),
                                            timeRefsClaim.split('\t'),timeHeidelClaim.split('\t'))
        ...
        for i in range(len(evidences)):
            ...
            timeRefsSnippet = timeRefsSnippets[i].split('\t')
            timeHeidelSnippet = timeHeidelSnippets[i].split('\t')
            evidence_encoding, evidence_EncodingWithoutTime, timeEvidences = self.encoder.forwardAttribution(
            ...
            evidenceEncodings.append((evidence_encoding, evidence_EncodingWithoutTime, timeEvidences))
            ...
            label_distributionDomain = self.labelMaskDomain(label_distribution).to(self.device)
            distribution = distribution + torch.mul(rank_evidence, label_distributionDomain).to(self.device)
        return distribution.to(self.device), claim_encodingFull, claim_EncodingWithoutTime, times, evidenceEncodings
    ...
    def forwardIntegrated(self, inputs, metadata_encoding):
        if len(inputs[1]) > 0:
            ...
            for j in range(len(inputs[1])):
                sum += inputs[1][j]
                number += 1
            claimEncoding = self.alpha * inputs[0] + (1-self.alpha) * sum / number
        else:
            claimEncoding = inputs[0]
        distribution = torch.zeros(len(self.labelDomains[self.domain])).to(self.device)
        for i in range(len(inputs[2])):
            if len(inputs[2][i][1]) > 0:
                sum = torch.zeros(len(inputs[2][i][1][0]))
                number = 0
                for j in range(len(inputs[2][i][1])):
                    sum += inputs[2][i][1][j]
                    number += 1
                evidence_encoding = self.alpha * inputs[2][i][0] + (1-self.alpha) * sum / number
            else:
                evidence_encoding = inputs[2][i][0]
    ...
    '''
    Generate random baseline where output for every label is below 1e-5
    '''
...
if __name__ == "__main__":
    ...
    index = 0
    encoderM = encoder(300, 128,sys.argv[2]).to(device)

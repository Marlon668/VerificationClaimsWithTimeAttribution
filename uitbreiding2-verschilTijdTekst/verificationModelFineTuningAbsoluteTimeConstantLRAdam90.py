...
from encoderClaimAbsoluteTime90 import encoderAbsolute
...
class verifactionModel(nn.Module):
    ...
    def saving_NeuralNetwork(model):
        pathI = "/scratch/leuven/347/vsc34763/modelsAbsoluteTestConstantLRAdam90/" + model.domain
...
def preprocessing(models,epochs=100):
    with torch.no_grad():
        for model in models:
            ...
            file = open('/scratch/leuven/347/vsc34763/lossAbsoluteTest2ConstantLRAdam90/' + 'Loss' + model[3], 'a', encoding='utf-8')
            ...
            if not model[8].early_stop:
                if model[8].better:
                    file = open('/scratch/leuven/347/vsc34763/epochsAbsoluteTest2ConstantLRAdam90/' + model[3], 'w', encoding='utf-8')
            ...
            for model in removeModels:
                ...
                if model not in removeEntirely:
                    ...
                    validation_loss, microF1Test, macroF1Test = eval_loop(model[11], model[1], oneHotEncoderM,
                                                                          ...
                                                                          domainLabelIndices, device)
                    model[8](validation_loss, microF1Test, macroF1Test, model[1])
                    file = open('/scratch/leuven/347/vsc34763/finalResults/iteratie2/lossAbsoluteTest2ConstantLRAdam90A/' + 'Loss' + model[3], 'a', encoding='utf-8')
                    file.write("Stap " + str(it) + " - " + str(validation_loss) + "\n")
                    file.close()
                    if not model[8].early_stop and it < epochs:
                        if model[8].better:
                            file = open('/scratch/leuven/347/vsc34763/finalResults/iteratie2/epochsAbsoluteTest2ConstantLRAdam90A/' + model[3], 'w', encoding='utf-8')
                            file.write('Micro - ' + str(microF1Test) + '\n')
                            file.write('Macro - ' + str(macroF1Test) + '\n')
                            file.write(model[8].lastResult + '\n')
                            file.write(str(it))
                    ...
                    file = open('/scratch/leuven/347/vsc34763/finalResults/iteratie2/lossAbsoluteTest2ConstantLRAdam90A/' + 'LossTrain' + model[3], 'a',
                                encoding='utf-8')

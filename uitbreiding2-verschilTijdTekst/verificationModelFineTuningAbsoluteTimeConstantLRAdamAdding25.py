...
from encoderClaimAbsoluteTimeAdding25 import encoderAbsolute
...
class verifactionModel(nn.Module):
    ...
    def saving_NeuralNetwork(model):
        pathI = "/scratch/leuven/347/vsc34763/finalResults/iteratie2/modelsAbsoluteTestConstantLRAdamAdding25A/" + model.domain
...
def preprocessing(models,epochs=600):
    with torch.no_grad():
        for model in models:
            ...
            file = open('/scratch/leuven/347/vsc34763/finalResults/iteratie2/lossAbsoluteTest2ConstantLRAdamAdding25A/' + 'Loss' + model[3], 'a', encoding='utf-8')
            ...
            if not model[8].early_stop:
                if model[8].better:
                    file = open('/scratch/leuven/347/vsc34763/finalResults/iteratie2/epochsAbsoluteTest2ConstantLRAdamAdding25A/' + model[3], 'w', encoding='utf-8')
            ...
            for model in removeModels:
                ...
                if model not in removeEntirely:
                    ...
                    if not model[8].early_stop and it < epochs:
                        if model[8].better:
                            file = open('epochsAbsoluteTestDecay/' + model[3], 'w', encoding='utf-8')
                            file.write('Micro - ' + str(microF1Test) + '\n')
                            file.write('Macro - ' + str(macroF1Test) + '\n')
                            file.write(model[8].lastResult + '\n')
                            file.write(str(it))
                    ...
                    file = open('/scratch/leuven/347/vsc34763/finalResults/iteratie2/epochsAbsoluteTest2ConstantLRAdamAdding25A/' + 'LossTrain' + model[3], 'a', encoding='utf-8')

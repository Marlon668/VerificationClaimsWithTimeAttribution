...
class verifactionModel(nn.Module):
        ...
        if positions[0] != "":
            for position in positions:
                ...
                if int(position[0]) <512:
                    inputForward[0][int(position[0])] = alpha*inputForward[0][int(position[0])] + (1-alpha)* self.positionEmbeddings(
                        torch.tensor([int(position[1]) + 100]).to(self.device)).squeeze(0).to(self.device)
        if verbs[0] != "":
            for verb in verbs:
                if int(verb) < 512:
                    inputForward[0][int(verb)] = alpha*inputForward[0][int(verb)] + (1-alpha)* self.predicateEmbeddings(torch.tensor([0]).to(self.device)).squeeze(0).to(
                        self.device)
        if times[0] != "":
            for time in times:
                if int(time) < 512:
                    inputForward[0][int(time)] = alpha*inputForward[0][int(time)]+(1-alpha)* self.predicateEmbeddings(torch.tensor([1]).to(self.device)).squeeze(0).to(
                        self.device)
                ...
                if int(index) < 512:
                    if verschillenValues[i].find('Duur') == -1 and verschillenValues[i].find('Refs') == -1:
                        if verschillenValues[i].isdigit():
                            inputForward[0][int(index)] = alpha*inputForward[0][int(index)]+(1-alpha)* self.verschil(
        ...
        for i in range(len(evidences)):
            ...
            if verbsSnippet[0] != "":
                for verb in verbsSnippet:
                    if int(verb)<512:
                        inputForward[0][int(verb)] = alpha*inputForward[0][int(verb)]+(1-alpha)* self.predicateEmbeddings(torch.tensor([0]).to(self.device)).squeeze(
                            0).to(
                            self.device)
            if timeExpressionsSnippet[0] != "":
                for time in timeExpressionsSnippet:
                    if int(time)<512:
                        inputForward[0][int(time)] = alpha*inputForward[0][int(time)]+(1-alpha)* self.predicateEmbeddings(torch.tensor([1]).to(self.device)).squeeze(
                            0).to(
                            self.device)
    ...
    def saving_NeuralNetwork(model):
        pathI = "/scratch/leuven/347/vsc34763/modelsBERTAbsoluteAdamW10/" + model.domain
...
def preprocessing(models,epochs=100):
    with torch.no_grad():
        for model in models:
            model[1].eval()
            ...
            file = open('/scratch/leuven/347/vsc34763/lossAbsoluteBertAdamW10/' + 'Loss' + model[3], 'a', encoding='utf-8')
            ...
            file = open('/scratch/leuven/347/vsc34763/lossAbsoluteBertAdamW/' + 'LossTrain' + model[3], 'a', encoding='utf-8')
            file.write("Preprocessing stap " + str(0) + " - " + str(validation_lossT) + "\n")
            file.close()
            '''
            if not model[8].early_stop:
                if model[8].better:
                    file = open('/scratch/leuven/347/vsc34763/epochsAbsoluteBertAdamW10/' + model[3], 'w', encoding='utf-8')
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        for model in domainModels:
            ...
            selector = torch.optim.lr_scheduler.ExponentialLR(optimizer, gamma=0.9)
            models.add(
                tuple([iter(model[0]), model[3], optimizer, model[5],
                       model[6], model[2], 0, model[0], early_stopping,
                       optimizer, model[1], selector]))
            ...
            for model in removeModels:
                ...
                if model not in removeEntirely:
                    ...
                    if not model[8].early_stop and it < epochs:
                        if model[8].better:
                            file = open('epochsAbsoluteBert/' + model[3], 'w', encoding='utf-8')
                            file.write('Micro - ' + str(microF1Test) + '\n')
                            file.write('Macro - ' + str(macroF1Test) + '\n')
                            file.write(model[8].lastResult + '\n')
                            file.write(str(it))
                    ...
                    losses[model[4]] = 0
                    file = open('lossAbsoluteBert/' + 'LossTrain' + model[3], 'a', encoding='utf-8')
                    file.write("Stap " + str(it) + " - " + str(losses[model[4]]) + "\n")
                    file.close()
                    losses[model[4]] = 0
                    model[12].step()

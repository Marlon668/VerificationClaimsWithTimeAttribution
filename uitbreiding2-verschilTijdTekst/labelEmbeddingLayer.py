...
class labelEmbeddingLayer(nn.Module):
    ...
    def __init__(self,embedding_dim, labelDomains):
        ...
        #size = int((embedding_dim-16)/2)
        #self.padding = nn.ConstantPad1d(size, 1)
    ...
    def forward(self, input,domain):
        ...
        #return (label_distribution-torch.min(label_distribution))/(torch.max(label_distribution)-torch.min(label_distribution))
